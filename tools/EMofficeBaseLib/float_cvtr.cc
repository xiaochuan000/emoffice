...
cVtr cVtr::operator*(const vtr &operand2) const
{
  ...
  product.setcvtr(y * (float)operand2.getz() - z * (float)operand2.gety(),
		  z * (float)operand2.getx() - x * (float)operand2.getz(),
		  x * (float)operand2.gety() - y * (float)operand2.getx());
...
Complex dotP(cVtr cv, vtr vv)
{
  ...
  dp = cv.getx() * (float)vv.getx() +
    cv.gety() * (float)vv.gety() +
    cv.getz() * (float)vv.getz();
